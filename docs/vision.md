...
Продукт обрабатывает поступающие данные, выносящий результирующее число в Психопаспорт. Также данный продукт выносит окончательное решение о допустимых мерах воздействия на преступника в случае его задержания, руководствуясь коэффициентом преступности и непосредственно переводящий Доминатор из одного режима в другой.  
...
На основе поступающий данных выдаёт коэффициент преступности человека в Психопаспорт
...
Предоставление доступа к оружию Доминатор
  ...
  Система решает кому и когда предоставлять доступ к оружию Доминатор, на основе коэффициента преступления и переводит Доминатор из одного режима в другой
...
*[В данном разделе зафиксируйте все ценовые факторы, которые могут повлиять на сбыт продукта - например, цена носителей для распространения, цена печати руководства пользователя, упаковки и т.п.]*  
  * Лицензия на Java SE 8.0 - $25 per month per processor.
  * Oracle DBMS - Oracle price list.  
...
Установка на сервера с ОС Solaris 10 (Standard Edition) возможна в виде:
 * копирования jar архива с компьютера разработчика;
 * компиляции из исходного кода под управлением maven при помощи java компилятора;  
 
Помимо этого, системой будет использоваться платформа Java SE 8.0, СУБД Oracle (до 100 пользователей и до 12 процессоров).  
Во время разработки будет использоваться git (GNU General Public License version 2.0) и maven (Apache 2.0 License). На компьютерах разработчиков будут использоваться  ОС Ubuntu и Windows 10 Pro.
...
*[Перечислите и кратко опишите особенности продукта - возможности системы, которые необходимо реализовать для того, чтобы система приносила пользу пользователям. При описании не стоит вдаваться в технические детали, оно должно быть всем понятно, лучше фокусироваться на обеспечиваемом функционале и на причинах необходимости его разработки. Каждая особенность должна быть подробно описана в Описании Прецедента.]*  
 ...
 5.1 Оценка психического состояния, личности и вероятность того, что человек будет совершать преступления на основе Психопаспорта, Интегральная характеристика (выраженная в виде числа или отображённая соответствующим цветом)  
 5.2 Выносение окончательного решения о допустимых мерах воздействия на преступника в случае его задержания, руководствуясь коэффициентом преступности и непосредственно переводящая Доминатор из одного режима в другой.  
 5.3 Получение доступа к оружию Доминатор - высокотехнологичное оружие, используемое полицейскими Инспекторами и Исполнителями. Доминатор самостоятельно проводит сканирование и запрашивает незамедлительный анализ данных у Системы Сивилла.  
 ...
 *[Перечислите требования к архитектуре клиента и сервера, установленным программам и версиям библиотек.]*  
   ...
   Язык программирования сервера - Java 8.0.  
 ...
 *[В данном разделе определите границы параметров производительности, надежности, отказоустойчивости, удобства использования и т.п.]*  
  ...
  Система должна точно оценивать Коэффициент Преступности гражданина и выносить окончательное решение о допустимых мерах воздействия на преступника в случае его задержания 
